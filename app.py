...
async def register_user(
    ...
    if db_user:
        raise _fastapi.HTTPException(status_code=400, detail="Email already exists try again")
...
async def login_user(
        form_data: _security.OAuth2PasswordRequestFormAuth2PasswordRequestForm = _fastapi.Depends(),
    ...
    if not db_user:
        raise _fastapi.HTTPException(status_code=401, detail="Wrong Login Details")

    return await _services.create_token(db_user)

@app.get("/api/v1/users/current-user", response_model=_schema.UserResponse)
...
@app.put("/api/v1/posts/{post_id}/"), response_model=_schemas.PostRepsonse)
